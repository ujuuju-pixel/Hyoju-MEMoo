(() => {
  ...
  function pressGlyph(name, isConsonant) {
    ...
    tokens.push({ type:'glyph', name, ssang: ssangMode && isConsonant });
    if (ssangMode) { ssangMode=false; els.ssang.classList.remove('active'); }
  ...
  els.ssang.addEventListener('click', () => { ssangMode=!ssangMode; batchimMode=false; els.ssang.classList.toggle('active',ssangMode); els.batchim.classList.remove('active'); });
  ...
  els.backspace.addEventListener('click', () => {
    ...
    if(!last) return;
    if(last.type==='glyph' && last.batchim) delete last.batchim; else tokens.pop();
    renderMemo(); scheduleSave();
  });
